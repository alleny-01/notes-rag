      ...
      <div ref={pdfScrollRef} className="mt-3 min-h-0 flex-1 overflow-y-auto overflow-x-hidden bg-[#e9e1d4] px-4 py-5 [overscroll-behavior-x:none] touch-pan-y [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {pdfError ? (
          ...
          <div ref={pageViewportRef} className="relative mx-auto w-fit max-w-full">
            <Document file={sourceUrl} loading={<SourceLoading />} onLoadSuccess={({ numPages }) => { setPageCount(numPages); setPageNumber((current) => Math.min(Math.max(current, 1), numPages)); }} onLoadError={(error) => { console.error("NotesRAG PDF viewer load failed", error); setPdfError("We could not open this PDF in the viewer. Your indexed passages remain available in chat."); }} className="mx-auto w-fit max-w-full">
              <Page pageNumber={resolvedPage} width={Math.min(720, typeof window === "undefined" ? 720 : Math.max(280, window.innerWidth > 1024 ? window.innerWidth * 0.32 : window.innerWidth - 64))} renderAnnotationLayer={false} renderTextLayer onRenderTextLayerSuccess={onRenderTextLayerSuccess} onRenderError={(error) => { console.error("NotesRAG PDF page render failed", error); setPdfError("We could not render this PDF page. Your indexed passages remain available in chat."); }} className="bg-white shadow-[0_12px_26px_rgba(66,47,39,0.16)]" />
            </Document>
